...
## Branching model
Trunk-based development: `main` is always deployable, all work happens on short-lived branches off `main`, merged back frequently. No long-lived `develop` or per-release branches — a release is just a tag on `main` (see Release Process in `HANDOFF.md`).
...
**Keeping a branch current:** if `main` has moved since your branch started and you need the update (or hit a conflict), merge `main` into your branch — don't rebase. Since PRs squash-merge (below), your branch's internal commit history disappears at merge time anyway, so there's no value in keeping it clean via rebase, only risk in rewriting history you might be sharing with a collaborator or an agent working the same branch.

**Force-pushing:** never on `main` (enforced by branch protection). On your own feature branch, force-pushing after a local amend/reset is fine as long as you're not overwriting a collaborator's or agent's in-flight commits on that same branch — check before you push if you're not sure you're the only one working it.

## Merge strategy
**Squash merge** on every PR into `main`. The PR's commits collapse into a single commit on `main`, using the PR title as the commit message — this is why the PR title must itself be a valid Conventional Commit (see below). Benefits:
- `main` history is linear and one-commit-per-change, which is what the changelog generator (Conventional Commits → CHANGELOG) reads
- Commit hygiene during a branch's lifetime doesn't matter — "wip", "fix typo", "actually fix it" commits are fine, they never reach `main`
- `git bisect` on `main` lands you on one complete, reviewed change per step
...
Since PR titles become `main`'s commit messages, get the title right before merging, not just the branch name.
...
The subagents in `.claude/agents/` and any local Claude Code session follow this same workflow: work on a properly prefixed branch, Conventional Commits format, never commit directly to `main`. Commits authored with agent assistance get a `Co-Authored-By:` trailer identifying the agent, same as any other tool-assisted commit.
