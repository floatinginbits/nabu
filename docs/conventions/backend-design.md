...
- **Repositories** do data access only. Implementation depends on the DB access pattern decision in `ARCHITECTURE.md` (`sqlc`/`GORM`/`sqlx`) — don't assume one before that's resolved.
