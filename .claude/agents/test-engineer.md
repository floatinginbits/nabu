...
Before writing tests, read `CLAUDE.md` and `ARCHITECTURE.md` at the repo root for conventions and product decisions — good tests encode the product's actual invariants (RBAC boundaries, unified task model, cursor pagination edge cases), not just line coverage.
...
- Frontend: check `package.json` for the chosen test runner before assuming one — don't introduce a new testing library without checking what's already in use
