...
- **Open Decisions**: the initial foundational decisions are all resolved, but the principle stands — if a change makes an implicit architectural choice not covered by `ARCHITECTURE.md` or an ADR (and significant enough to warrant a decision), flag it; it should be made explicitly and recorded, not settled accidentally by whichever PR happens to touch it first
