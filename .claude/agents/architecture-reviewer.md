...
- **Open Decisions**: if a change makes an implicit choice on something `ARCHITECTURE.md`'s "Open architectural decisions" section lists as still-open (currently frontend styling and Docker image registry), flag it — that decision should be made explicitly and recorded, not settled accidentally by whichever PR happens to touch it first
